...
@implementation PMTweenEasingQuintic
...
+ (PMTweenEasingBlock)easingIn {
    ...
    PMTweenEasingBlock easing = ^double (NSTimeInterval elapsedTime, double startValue, double valueDelta, NSTimeInterval duration) {
        ...
        double tween_value = valueDelta * (time*time*time*time*time) + startValue;
...
+ (PMTweenEasingBlock)easingOut {
    ...
    PMTweenEasingBlock easing = ^double (NSTimeInterval elapsedTime, double startValue, double valueDelta, NSTimeInterval duration) {
        ...
        double time = elapsedTime / duration;
        time--;
        double tween_value = valueDelta * (time*time*time*time*time + 1) + startValue;
...
+ (PMTweenEasingBlock)easingInOut {
    ...
    PMTweenEasingBlock easing = ^double (NSTimeInterval elapsedTime, double startValue, double valueDelta, NSTimeInterval duration) {
        ...
        if (time < 1) {
            tween_value = (valueDelta * 0.5) * time*time*time*time*time + startValue;
        } else {
            time -= 2;
            tween_value = (valueDelta * 0.5) * (time*time*time*time*time + 2) + startValue;
        }
